...
function prepareIdxAndDocMap() {
  const docs = [  
    ...
    } ,    
    {
      ...
      "content": "Number Validators Validators for Number are : Name Usage in composition syntax Usage in implicit syntax Description Error codes Minimal value minimalNumberValue(8) .min(8) Fails if value is greater or equal minimal value minimal_value Maximal value maximalNumberValue(8) .max(8) Fails if value is lower or equal maximal value maximal_value Equal value numberEqual(8) .equal(8) Fail if value is not exactly same value defined in function equal_field"
    } ,    
    {
      "title": "Option validators",
      "url": "/scala-simple-validation/docs/validators/option/",
      "content": "Option Validators Validators for Option are : Name Usage in composition syntax Usage in implicit syntax Description Error codes When value is defined ifDefined(&lt;validators&gt;) .ifDefined(&lt;validators&gt;) Runs passed to it validators when value is defined N\\A Is defined and N\\A definedAnd(&lt;validators&gt;) Fails if value is not defined and runs passed to it validators if value is defined empty_field If is defined isDefined .isDefined Fails if value is not defined empty_field If is not defined notDefined .notDefined Fails if value is defined empty_expected"
    } ,    
    {
      "title": "Schema definitions",
      "url": "/scala-simple-validation/docs/usage/schema/",
      "content": "Schema definitions You can create validation schema using composition or using implicits import pl.muninn.simple.validation.all._ case class Field(name:String, otherField:String) val compositionSchema:Schema[Field] = createSchema { context =&gt; context.field(_.name).is(noneEmptyString and stringMinimalLength(8)) + context.field(_.otherField).is(noneEmptyString) } // compositionSchema: Schema[Field] = &lt;function1&gt; compositionSchema.validate(Field(\"\",\"\")) // res0: cats.data.package.ValidatedNec[pl.muninn.simple.validation.InvalidField, Unit] = Invalid( // e = Append( // leftNE = Append( // leftNE = Singleton(a = EmptyField(field = \"name\")), // rightNE = Singleton( // a = MinimalLength(field = \"name\", expected = 8, value = 0) // ) // ), // rightNE = Singleton(a = EmptyField(field = \"otherField\")) // ) // ) val implicitSchema:Schema[Field] = createSchema { context =&gt; (context.field(_.name).noneEmptyString and stringMinimalLength(8)) + context.field(_.otherField).noneEmptyString } // implicitSchema: Schema[Field] = &lt;function1&gt; implicitSchema.validate(Field(\"\",\"\")) // res1: cats.data.package.ValidatedNec[pl.muninn.simple.validation.InvalidField, Unit] = Invalid( // e = Append( // leftNE = Append( // leftNE = Singleton(a = EmptyField(field = \"name\")), // rightNE = Singleton( // a = MinimalLength(field = \"name\", expected = 8, value = 0) // ) // ), // rightNE = Singleton(a = EmptyField(field = \"otherField\")) // ) // )"
    } ,      
    {
      "title": "String validators",
      "url": "/scala-simple-validation/docs/validators/string/",
      "content": "String Validators Validators for String are : Name Usage in composition syntax Usage in implicit syntax Description Error codes Empty string emptyString .emptyString Fails if string is not empty empty_expected None empty string noneEmptyString .noneEmptyString Fails if string is empty empty_field None empty string emailString .emailString Fails if string is not email email_field Complex password complexPassword .complexPassword Fails if string is not at least 8 characters long and contains 1 number and 1 special symbol and big and small letters password_complexity Minimal length stringMinimalLength(8) .minimalLength(8) Fails if length is greater or equal minimal value minimal_length Maximal length stringMaximalLength(8) .maximalLength(8) Fails if length is lower or equal maximal value maximal_length Expected length stringLength(8) .expectedLength(8) Fail if length is not exactly same value defined in function expected_length"
    } ,    
    {
      "title": "Usage",
